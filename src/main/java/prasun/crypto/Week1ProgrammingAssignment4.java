 ...
 */
public class Week1ProgrammingAssignment4 {
	static String MSGS[] = {
	...
	static {
		initAlphabetXorSpace((byte) 'a', (byte) 'z');
		initAlphabetXorSpace((byte) 'A', (byte) 'Z');
	}

	private static void initAlphabetXorSpace(byte from, byte to) {
	...
	public static void main(String[] args) {
		...
		for (String msg1 : MSGS) {
			for (String msg2 : MSGS) {
				if (msg1.length() > msg2.length())
					msg1 = msg1.substring(0, msg2.length());
				else
					msg2 = msg2.substring(0, msg1.length());
				byte[] msgByteArry = encode(hexToAscii(msg1), hexToAscii(msg2)).getBytes();
				byte[] msgByteArryCt1 = hexToAscii(msg1).getBytes();
				byte[] msgByteArryCt2 = hexToAscii(msg2).getBytes();
				...
				try {
					for (int j = 0; j < msgByteArry.length; j++) {
						...
						if (ALPHABET_XOR_MAP.containsKey(xorByte)) {
							...
							byte keypart1 = (byte) (msgByteArryCt1[j] ^ originalChar);
							byte keypart2 = (byte) (msgByteArryCt2[j] ^ originalChar);
		...
		int max_length = 0;
		for (String msg1 : MSGS) {
			if (msg1.length() > max_length)
				max_length = msg1.length();
		}
		final byte[] key = new byte[xorList.keySet().size()];
		for (int i = 0; i < xorList.keySet().size(); i++) {
		...
		for (String msg : MSGS) {
			...
			System.out.println(xorText(hexToAscii(msg), key));
		}
	}

	private static void decipherText(String cipheredText, byte[] key) {
		byte[] newKey = new byte[cipheredText.length() / 2];
		if (key.length > cipheredText.length() / 2)
			System.arraycopy(key, 0, newKey, 0, cipheredText.length() / 2);
		else {
			newKey = key;
			cipheredText = cipheredText.substring(0, key.length * 2);
		}
		BigInteger deciphered = new BigInteger(cipheredText, 16).xor(new BigInteger(newKey));
		System.out.println(new String(deciphered.toByteArray()));
	}

	static String xorText(String s, byte[] key) {
		StringBuilder sb = new StringBuilder();
		int length = Math.min(s.length(), key.length);
		for (int i = 0; i < length; i++)
			sb.append((char) (key[i] ^ s.charAt(i)));
		return sb.toString();
	}
